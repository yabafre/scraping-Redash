...
class ConfettiAnimation:
    ...
    def _animate(self):
        ...
        if self.message_text:
            ...
            # Texte du message (taille augmentée de 6px : 42 + 6 = 48px, maintenant +10px = 58px, +5px = 63px, +10px = 73px)
            self.canvas.create_text(
                center_x, center_y - 150,
                text=self.message_text,
                font=("Montserrat", 73, "bold"),
    ...
    def _build_ui(self):
        ...
        for i, ((r, c), title) in enumerate(zip(pos, titles)):
            ...
            title = ctk.CTkLabel(
                header_frame,
                text=title,
                font=("Montserrat", 49, "bold"),
                text_color="#000000",
                anchor="center"
            )
            title.pack(anchor="center")
            val = ctk.CTkLabel(frame, text="--", font=("Montserrat", 151, "bold"), text_color="#ffffff")
            val.pack(expand=True)
            if i == 0:
                inspiration = ctk.CTkLabel(
                    frame,
                    text="",
                    font=("Montserrat", 47, "italic"),
                    text_color="#888888"
                )
                inspiration.pack(pady=(0, 20), expand=True)
                self.q[i] = {"frame": frame, "val": val, "title": title, "inspiration": inspiration}
            else:
                trend = ctk.CTkLabel(frame, text="→", font=("Montserrat", 115), text_color="#ffffff")
                trend.pack(pady=6)
        ...
        test_confetti_btn = ctk.CTkButton(
            self.test_frame, text="Test Confettis", command=self._test_confetti, width=120, height=30, font=("Montserrat", 37))
        test_confetti_btn.pack(pady=2)
        toggle_test_btn = ctk.CTkButton(
            self.test_frame, text="Mode Test", command=self._toggle_test_mode, width=120, height=30, font=("Montserrat", 37))
        toggle_test_btn.pack(pady=2)
        simulate_btn = ctk.CTkButton(
            self.test_frame, text="Simuler Données", command=self._simulate_test_data, width=120, height=30, font=("Montserrat", 37))
        simulate_btn.pack(pady=2)
        reset_btn = ctk.CTkButton(
            self.test_frame, text="Reset", command=self._reset_test_state, width=120, height=30, font=("Montserrat", 37))
        ...
        self.celebration_label = ctk.CTkLabel(self.celebration_frame, text="", font=("Montserrat", 67, "bold"))
        self.celebration_label.pack(expand=True, pady=30)
        self.celebration_gift = ctk.CTkLabel(self.celebration_frame, image=None, text="", font=("Montserrat", 57))
        ...
        self.ts = ctk.CTkLabel(self, text="", font=("Montserrat", 43), text_color="#888888")
